import { Composition, NoteFunctionType } from "../lib/music-player";
...
export function jsonToComposition(
  json: JsonStave[],
  name: string,
  bpm: number,
): Composition {
...
function transposeTact(tact: JsonTact) {
  ...
  const result: Array<
    Array<{ value: string; duration: number; functionType: NoteFunctionType }>
  > = [];
  ...
  for (let colIdx = 0; colIdx < columnCount; colIdx++) {
    ...
    for (let stringIdx = 0; stringIdx < stringCount; stringIdx++) {
      const note = columns[colIdx].notes.find(
        (n) => n.stringNumber === stringIdx,
      );
      column.push({
        value: note?.value || "",
        duration: columns[colIdx].duration,
        functionType:
          (note?.functionType as NoteFunctionType) ?? NoteFunctionType.DEFAULT,
      });
    }
    result.push(column);
  }
